  ...
  const sendEmployeeIdWithLocation = async (employeeId: string) => {
    ...
    try {
      ...
      if (response.status) {
        ...
        setServerMessage(
          "位置情報エラー：許可されたスキャンエリアから離れすぎています。オフィス/大学内にいることを確認してください。"
        );
        ...
        >
          {isProcessing ? (
            <>
              ...
              <Typography variant="h5">お待ちください</Typography>
            </>
          ) : (
            <>
              {messageType === "error" ? (
              ...
              {serverMessage && (
                <Typography
                  variant="h5"
                  sx={{ marginTop: 2, color: getMessageColor() }}
                >
                  {serverMessage}
                </Typography>
              )}
              {employeeName && messageType !== "error" && (
                <Typography variant="h3" sx={{ marginTop: 2, color: "green" }}>
                  {messageType === "check-in" ? "ようこそ" : "お気をつけて"},{" "}
                  {employeeName}さん！
                </Typography>
              )}
